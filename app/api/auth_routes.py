...
def _issue_refresh(db: MySQLService, user_id: uuid.UUID, access_token: str, request: Optional[Request]) -> str:
    """Create a rotating refresh token tied to the access session."""
    svc = SessionService(db)
    return svc.issue_refresh_token(
        str(user_id),
...
<<<<<<< HEAD
    ...
    if fam:
        SessionService(db).revoke_family(fam["family_id"])
...
def refresh_session(
    ...
    Accepts the ``refresh_token`` httpOnly cookie or an ``X-Refresh-Token``
    header for non-browser clients. A replayed token revokes the whole family.
    """
    raw = request.cookies.get(REFRESH_COOKIE) or request.headers.get("x-refresh-token")
    if not raw:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Refresh token required")

    _enforce_rate_limit(request, "refresh", _client_ip(request), None)
    svc = SessionService(db)
    result = svc.rotate(raw, user_agent=request.headers.get("user-agent", ""), ip=_client_ip(request))
    if not result:
        _clear_auth_cookie(response)
        _clear_refresh_cookie(response)
    ...
    s = db.get_session()
    user = s.execute("SELECT * FROM users WHERE id=%s", (uuid.UUID(result.user_id),)).one()
    if not user:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="User not found")

    # Replace the old access session with a fresh one bound to the rotated token.
    old_access = request.cookies.get("access_token")
    if old_access:
        s.execute("DELETE FROM sessions WHERE token=%s", (_hash_token(old_access),))
    token = _issue_session(db, user.id)
    svc.attach_session_to_refresh(result.refresh_token, _hash_token(token))
    _set_auth_cookie(response, token)
...
def list_sessions(current=Depends(get_current_user), db: MySQLService = Depends(get_mysql)):
    """List active sessions (devices) for the signed-in user."""
    svc = SessionService(db)
    return {
        "sessions": svc.list_sessions(str(current["user"].id), _hash_token(current["token"])),
        "plan": svc.get_plan(str(current["user"].id)),
        "plans": PLANS,
    }
...
def revoke_one_session(
    session_id: str, current=Depends(get_current_user), db: MySQLService = Depends(get_mysql)
):
    if not re.fullmatch(r"[0-9a-f]{8,64}", session_id):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid session id")
    ok = SessionService(db).revoke_session(str(current["user"].id), session_id)
    if not ok:
...
def logout_everywhere(
    ...
    """Sign out every device. ``keep_current=false`` also ends this session."""
    keep = _hash_token(current["token"]) if keep_current else None
    count = SessionService(db).revoke_all(str(current["user"].id), except_token_hash=keep)
    if not keep_current:
        _clear_auth_cookie(response)
        _clear_refresh_cookie(response)
